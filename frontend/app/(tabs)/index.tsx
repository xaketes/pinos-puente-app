...
import { EmptyState, Header, Screen, SectionTitle, sharedStyles } from "@/src/components/screen";
import { useApp } from "@/src/store";
import { makeStyles, useTheme } from "@/src/theme";

export default function ConvocatoriaScreen() {
  const { state, hydrated, setAttendance, setMatch, prepareNextMatch } = useApp();
  ...
  return <Screen testID="convocatoria-screen">
    ...
    <View style={[sharedStyles.card, styles.matchCard, { backgroundColor: colors.surfaceSecondary, borderColor: colors.border }]}>
      <View style={styles.fieldRow}><View style={styles.field}><Text style={[sharedStyles.fieldLabel, { color: colors.muted }]}>Fecha</Text><TextInput testID="match-date" value={date} onChangeText={(value) => { setDate(value); setMatch({ date: value }); }} placeholder="DD / MM / AAAA" placeholderTextColor={colors.muted} style={[sharedStyles.input, { color: colors.onSurface, backgroundColor: colors.surfaceTertiary, borderColor: colors.border }]} /></View><View style={styles.field}><Text style={[sharedStyles.fieldLabel, { color: colors.muted }]}>Hora</Text><TextInput testID="match-time" value={time} onChangeText={(value) => { setTime(value); setMatch({ time: value }); }} placeholder="20:30" placeholderTextColor={colors.muted} style={[sharedStyles.input, { color: colors.onSurface, backgroundColor: colors.surfaceTertiary, borderColor: colors.border }]} /></View></View>
      <Text style={[sharedStyles.fieldLabel, { color: colors.muted }]}>Pabellón</Text><TextInput testID="match-venue" value={venue} onChangeText={(value) => { setVenue(value); setMatch({ venue: value }); }} placeholder="Nombre del pabellón" placeholderTextColor={colors.muted} style={[sharedStyles.input, { color: colors.onSurface, backgroundColor: colors.surfaceTertiary, borderColor: colors.border }]} />
    </View>
    ...
    {state.players.length === 0 ? <EmptyState icon="account-plus-outline" title="La plantilla está vacía" message="Añade tus jugadores desde Administración para empezar la convocatoria." /> : state.players.map((player) => <View key={player.id} style={[styles.playerCard, { backgroundColor: colors.surfaceSecondary, borderColor: colors.border }]}>
      ...
      <View style={styles.attendanceActions}><Pressable testID={`voy-${player.id}`} accessibilityRole="button" onPress={() => setAttendance(player.id, "yes")} style={[styles.attendanceButton, { backgroundColor: player.attendance === "yes" ? colors.success : colors.surfaceTertiary }]}><Text style={{ color: player.attendance === "yes" ? colors.onSuccess : colors.muted, fontWeight: "900", fontSize: 12 }}>VOY</Text></Pressable><Pressable testID={`no-voy-${player.id}`} accessibilityRole="button" onPress={() => setAttendance(player.id, "no")} style={[styles.attendanceButton, { backgroundColor: player.attendance === "no" ? colors.error : colors.surfaceTertiary }]}><Text style={{ color: player.attendance === "no" ? colors.onError : colors.muted, fontWeight: "900", fontSize: 12 }}>NO VOY</Text></Pressable></View>
    </View>)}
  </Screen>;
}
